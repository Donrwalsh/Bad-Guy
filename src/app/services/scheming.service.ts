...
export class SchemingService extends BaseNum {
    ...
    get schemePointsHatchedThisSecond() {
        ...
        if (Base.BANKED_SCHEME_POINTS >= 1 && Base.EARNING_SCHEME_POINTS) {
            ...
            bankHatch = bankHatch > Base.BANKED_SCHEME_POINTS ? Math.floor(Base.BANKED_SCHEME_POINTS) : bankHatch;
    ...
    earnSchemePoints(num) {
        ...
        if (Base.CURRENT_SCHEME.exp >= Base.CURRENT_SCHEME.currentExpTarget){
            if (this.mathematicsUnlocked) {
                var remainder = Base.CURRENT_SCHEME.exp - Base.CURRENT_SCHEME.currentExpTarget;
